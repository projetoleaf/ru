...
public class TipoValorController {
	...
	@PostMapping("/salvar")
    public String salvarTipoValor(Model model, @ModelAttribute("tipoValor") @Valid TipoValor tipoValor, BindingResult result) {
		try {
            if (!result.hasErrors()) {
                TipoValor tipoAtualizado = repository.save(tipoValor);
                log.info(tipoAtualizado.toString() + " gravado com sucesso");
                model.addAttribute("mensagemInfo", config.getMessage("gravadoSucesso", new Object[] { "o tipo valor" }, null));
            }
        }
        catch (Exception ex) {
    ...
    public String excluirTipoValor(RedirectAttributes ra, @PathVariable Long id) {		
		try {
            ...
            ra.addFlashAttribute("mensagemInfo", config.getMessage("excluidoSucesso", new Object[] { "o tipo valor" }, null));
        }
        catch (Exception ex) {
            log.error("Erro de processamento", ex);

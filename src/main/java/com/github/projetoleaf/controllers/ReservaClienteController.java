...
public class ReservaClienteController {
	...
	public String reservaRefeicoes(Model model)
		...
		if (!(authentication instanceof AnonymousAuthenticationToken)) {
			...
			// (dataHoje.get(Calendar.DAY_OF_WEEK) <= dataSemana.get(Calendar.DAY_OF_WEEK)))
			// {
			if (dataHoje.getTime().equals(diaDaSemana.getTime()) && horaDeHoje >= 7
					&& dataHoje.get(Calendar.DAY_OF_WEEK) <= penultimoDiaUtil.get(Calendar.DAY_OF_WEEK)) {
				...
				for (int i = 0; i < 5; i++) {
					...
					for (Object[] linhaDoBanco : dataDoBanco) {
						...
						if (countSegunda < 360 || countTerca < 360 || countQuarta < 360 || countQuinta < 360
								|| countSexta < 360) {
			...
			} else if (!dataHoje.getTime().equals(diaDaSemana.getTime())
			// && dataHoje.get(Calendar.DAY_OF_WEEK) <=
			// penultimoDiaUtil.get(Calendar.DAY_OF_WEEK)
			) {
	...
	public String salvarReservaRefeicoes(@RequestParam("data") String[] idsCardapios,
			@RequestParam("tipoRefeicao") Integer[] idsTipoRefeicao, @RequestParam("pagamento") String pag) {

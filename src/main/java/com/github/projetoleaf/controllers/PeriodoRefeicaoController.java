...
public class PeriodoRefeicaoController {
	...
	@PostMapping("/salvar")
    public String salvarPeriodoRefeicao(Model model, @ModelAttribute("periodoRefeicao") @Valid PeriodoRefeicao periodoRefeicao, BindingResult result) {
		try {
            if (!result.hasErrors()) {
            	periodoRefeicao = repository.save(periodoRefeicao);
                log.info(periodoRefeicao.toString() + " gravado com sucesso");
                model.addAttribute("mensagemInfo", config.getMessage("gravadoSucesso", new Object[] { "o período refeição" }, null));
            }
        }
        catch (Exception ex) {
    ...
    public String excluirPeriodoRefeicao(RedirectAttributes ra, @PathVariable Long id) {		
		try {
            ...
            ra.addFlashAttribute("mensagemInfo", config.getMessage("excluidoSucesso", new Object[] { "o período refeição" }, null));
        }
        catch (Exception ex) {
            log.error("Erro de processamento", ex);

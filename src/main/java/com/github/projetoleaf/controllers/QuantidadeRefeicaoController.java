...
public class QuantidadeRefeicaoController {
	...
	@PostMapping("/salvar")
    public String salvarQuantidadeRefeicao(Model model, @ModelAttribute("quantidadeRefeicao") @Valid QuantidadeRefeicao quantidadeRefeicao, BindingResult result) {
		try {
            if (!result.hasErrors()) {
            	quantidadeRefeicao = repository.save(quantidadeRefeicao);
                log.info(quantidadeRefeicao.toString() + " gravada com sucesso");
                model.addAttribute("mensagemInfo", config.getMessage("gravadoSucesso", new Object[] { "a quantidade de refeição" }, null));
            }
        }
        catch (Exception ex) {
    ...
    public String excluirQuantidadeRefeicao(RedirectAttributes ra, @PathVariable Long id) {		
		try {
            ...
            ra.addFlashAttribute("mensagemInfo", config.getMessage("excluidoSucesso", new Object[] { "a quantidade de refeição" }, null));
        }
        catch (Exception ex) {
            log.error("Erro de processamento", ex);
